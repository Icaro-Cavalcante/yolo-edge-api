...
def parse_args():
    ...
    p.add_argument("--threshold", type=float, default=40.0, help="Threshold do Laplaciano para nitidez")
    return p.parse_args()
...
def main():
    ...
    try:
        while saved < args.total:
            ...
            print(f"  [{saved:>3}/{args.total}] Salvo: {path.name} ", f"(descartados: {skipped})", end=end_char)
